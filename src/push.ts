...
import { TezosNodeWriter, TezosParameterFormat, TezosNodeReader, KeyStore, Transaction } from 'conseiljs';
...
export default async function pushOracleData(
    ...
    try {
        ...
        const operation = await constructPushOperation(
            logLevel,
            keystore,
            counter + 1,
        ...
        Utils.print("Push sent with hash: " + nodeResult.operationGroupID.replace(/"/g, ""))
    } catch (error) {
