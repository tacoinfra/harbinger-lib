...
import { TezosNodeReader, TezosNodeWriter, TezosParameterFormat, KeyStore, Signer, TezosLanguageUtil, Transaction } from 'conseiljs';
...
export default async function updateOracleFromCoinbase(
    ...
    if (updateIntervalSeconds) {
        ...
        while (true) {
            ...
            Utils.print(`Waiting ${updateIntervalSeconds} seconds to do next update. (Customize with --updateInterval)`)
...
export async function updateOracleFromCoinbaseOnce(
    ...
    try {
        ...
        if (medianizerContractAddress !== undefined) {
            const medianizerPushOperation = await constructPushOperation(
                logLevel,
                keyStore,
                counter + 2,
                oracleContractAddress,
...
async function makeUpdateOperationFromCoinbase(
    ...
    const elementString = elements.reduce((previousValue, element) => {
...
async function makeUpdateOperationFromFeed(
    ...
    // Retrieve elements from the oracle data.
    const oracleData = await retrieveOracleDataFromFeed(
        oracleFeedURL
    )
    if (logLevel == LogLevel.Debug) {
        Utils.print("Received oracle data: ")
 ...
 */
async function retrieveOracleDataFromFeed(
    oracleFeedURL: string
): Promise<any> {
    ...
    let oracleData = JSON.parse(oracleDataRaw.content)
...
function parseRawOracleData(oracleData: any): any {
    ...
    const result = {
        messages: oracleData.messages.map((message: string) => {
            ...
            const normalized = TezosLanguageUtil.normalizeMichelsonWhiteSpace(parsed.code)
            console.log("done")
 ...
 */
function listElementForAsset(oracleData: OracleData, assetName: string): string {
    for (var i = 0; i < oracleData.messages.length; i++) {
    ...
    throw new Error(`Could not locate ${assetName} in ${oracleData.messages}`)
}
