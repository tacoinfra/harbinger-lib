...
import { KeyStore, Signer, TezosMessageUtils, TezosNodeReader, TezosNodeWriter } from 'conseiljs'
...
const utils = {
    ...
    async signerFromKeyStore(keyStore: KeyStore): Promise<Signer> {
        let bytes = TezosMessageUtils.writeKeyWithHint(keyStore.secretKey, 'edsk')
     ...
     */
    async revealAccountIfNeeded(tezosNodeURL: string, keyStore: KeyStore, signer: Signer) {
        const publicKeyHash = keyStore.publicKeyHash
        if (await TezosNodeReader.isManagerKeyRevealedForAccount(tezosNodeURL, publicKeyHash)) {
            return
        }

        this.print(`Account ${publicKeyHash} is not revealed. Sending a one time operation to reveal the account.`)
        const result = await TezosNodeWriter.sendKeyRevealOperation(tezosNodeURL, signer, keyStore, undefined)
        ...
        while (!isRevealed) {
            ...
            isRevealed = await TezosNodeReader.isManagerKeyRevealedForAccount(tezosNodeURL, publicKeyHash)
        }
        this.print(`All done!`)
        this.print(``)
    },
     ...
     */
    calculateContractAddress(operationHash: string, index: number) {
